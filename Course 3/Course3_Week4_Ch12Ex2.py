...
# Actual problem: Start at: http://py4e-data.dr-chuck.net/known_by_Tamarah.html
...
#Lets grab the first set of URLs
...
#Now lets use the user inputs to cycle through the links
...
while count > 0:
    html = urllib.request.urlopen(tags[position].get('href',None), context=ctx).read()
    soup = BeautifulSoup(html, 'html.parser')
